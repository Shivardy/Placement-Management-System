...
<html lang="en">
 ...
 window.location.href = window.location.href + "?sort=" + sort ;
	...
	window.location.href = window.location.href + "&sort=" + sort ;
	 ...
	 window.location.href ="http://localhost:12/placements/admin/staff" + "?sort=" + sort ;
	}
...
<div class="list-group">
  ...
  <a href="stats" class="list-group-item">Statistics</a>
	...
	<div class="panel panel-primary">
	<div class="panel-heading">
	 <h1 align="center"class="panel-title">View Staff</h1></div>
...
$con=mysqli_connect($host,$user,$password,$db_name);
	...
	$query='UPDATE staff SET accountstatus="deactivate" WHERE id="'.$_GET['deactivateid'].'"';
$con->query($query);
	}
	if(isset($_GET['activateid'])){
	$query='UPDATE staff SET accountstatus="activate" WHERE id="'.$_GET['activateid'].'"';
$con->query($query);
	}
  ...
  $query = "SELECT count(id) FROM `staff` WHERE CONCAT(`firstname`, `lastname`,`branch`,`contactno`,`email`,`accountstatus`) LIKE '%".$_GET['search']."%'";
$run = $con->query($query);
...
$sort=$_GET['sort'];	
		  ...
		  $query = "SELECT * FROM `staff` WHERE CONCAT(`firstname`, `lastname`,`branch`,`contactno`,`email`,`accountstatus`) LIKE '%".$_GET['search']."%' ORDER BY $sort $limit";
$run = $con->query($query);
$pagination='';
...
<div class="col-sm-3">
	 	 <select id="mySelect" onchange="myFunction()" name="sort"><option><b>Sort By</b></option><option value="firstname">Staff name</option><option value="branch">Department</option><option value="contactno">Contactno</option><option value="email">Email</option><option value="accountstatus">Account status</option></select></div>
	<div class="col-sm-5">
...
echo "<tr><td>".$row['firstname']." ".$row['lastname']."</td><td>".$row['branch']."</td><td>".$row['contactno']."</td><td>".$row['email']."</td>";


	echo '<td><a href="http://localhost:12/placements/admin/messaging?stid='.$row['id'].'"><button class="btn btn-default btn-xs">Send msg</button></td>';
...
echo "</tbody></table> <hr></div>";
	...
	echo "<br><br></div>";
		 ...
		 $query="SELECT COUNT(id) FROM staff";
$run = $con->query($query);
$row=mysqli_fetch_array($run);
	$rows=$row[0];
	$pagerows=8;
...
$sort=$_GET['sort'];	
		...
		$query="SELECT * FROM staff ORDER BY $sort $limit ";
$run = $con->query($query);
$pagination='';
