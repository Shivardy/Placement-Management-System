...
<div class="list-group">
  ...
  <a href="stats" class="list-group-item">Statistics</a>
	...
	<div class="panel panel-primary">
	<div class="panel-heading">
	 <h1 align="center"class="panel-title">View Applications</h1></div>
...
<div class="col-sm-3">
	 	 <select id="mySelect" onchange="myFunction()" name="sort"><option><b>Sort By</b></option><option value="student">Student Rollno</option><option value="branch">Branch</option><option value="company">Company</option><option value="jobtitle">Job Title</option><option value="applydate">Applied Date</option></select></div>
	<div class="col-sm-5">
...
while($row=$run->fetch_assoc()){
	...
	echo "<tr><td>".$row['student']."</td><td> ".$row['branch']."</td><td>".$row['company']."</td><td>".$row['jobtitle']."</td><td>".$row['applydate']."</td>";
	echo '<td><a href="http://localhost:12/placements/admin/viewapp?user='.$row['student'].'&job='.$row['jobtitle'].'&company='.$row['company'].'"><button class="btn btn-default btn-xs">Download</button></a></td>';
...
$sort=$_GET['sort'];	
		...
		$query="SELECT * FROM jobs ORDER BY $sort desc $limit ";
$run = $con->query($query);
$pagination='';
