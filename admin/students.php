...
<html lang="en">
 ...
 window.location.href = window.location.href + "&sort=" + sort ;
	...
	window.location.href = window.location.href + "&sort=" + sort ;
	 ...
	 window.location.href ="http://localhost:12/placements/admin/students" + "?sort=" + sort ;
	}
...
$con=mysqli_connect($host,$user,$password,$db_name);
	...
	$query='UPDATE students SET accountstatus="deactivate" WHERE id="'.$_GET['deactivateid'].'"';
$con->query($query);
	}
	if(isset($_GET['activateid'])){
	$query='UPDATE students SET accountstatus="activate" WHERE id="'.$_GET['activateid'].'"';
$con->query($query);
	}
  ...
  $query = "SELECT count(id) FROM `students` WHERE CONCAT(`firstname`, `lastname`,`branch`,`city`,`gender`,`placementstatus`,`accountstatus`) LIKE '%".$_GET['search']."%'";
$run = $con->query($query);
...
$sort=$_GET['sort'];	
		  ...
		  $query = "SELECT * FROM `students` WHERE CONCAT(`firstname`, `lastname`,`branch`,`city`,`gender`,`placementstatus`,`accountstatus`) DESC LIKE '%".$_GET['search']."%' ORDER BY $sort $limit ";
$run = $con->query($query);
$pagination='';
...
while($row=$run->fetch_assoc()){
	...
	else {echo  '<td>'.$row['placementstatus'].'<a href="http://localhost:12/placements/admin/change_status?id='.$row['id'].'"><button class="btn btn-warning btn-xs">Change Status</button></a></td>'; }
